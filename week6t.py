...
"""
import sqlite3
conn = sqlite3.connect(r"D:\Gatchaiyo_python\week6test.db")
c = conn.cursor()
c.execute ('''CREATE TABLE Allstudent (No integer PRIMARY KEY AUTOINCREMENT,
...
def puss(): # เพิ่มข้อมูลนักเรียน
    ...
    while z == "y":
        ...
        try :
            ...
            c.execute('INSERT INTO Allstudent (number,fname,lname,email,sex,old,year) VALUES (?,?,?,?,?,?,?)',data)
            conn.commit()
            conn.close()
...
def show(): #แสดงข้อมูลนักเรียน
    ...
    conn = sqlite3.connect(r"D:\Gatchaiyo_python\week6test.db")
    c = conn.cursor()
    c.execute('''SELECT * FROM Allstudent''')
...
def delete(): #ลบข้อมูลนักเรียน
    ...
    while z == "y":
        ...
        c = conn.cursor()
        try :
            c.execute("DELETE FROM Allstudent WHERE No ="+str(n1))
            conn.commit()
            conn.close()
...
DELETE FROM sqlite_sequence WHERE name = 'Allstudent';
"""